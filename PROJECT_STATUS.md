...
Owner decision: whether a separate controlled QA mechanism is necessary for live `low_confidence` and `forbidden_authority_fields` scenario verification. Until that decision, do not move to OpenAI integration, enable live OpenAI, add API keys, change DNS/custom domains, connect the endpoint to production Business Idea Validator, or turn shadow mode on by default. The next Session 08 starting point must be decided after the Session 07 checkpoint; do not start it from this checkpoint.
